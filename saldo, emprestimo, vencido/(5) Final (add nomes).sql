...
FROM (
    ...
    FROM (
        SELECT
            (
                (
                    FACMUTUO.FACCOR_FUNCTIONS.PEGASALDODIA(D.CONTAC, '24/08/2023')
                    + FACMUTUO.FACCOR_FUNCTIONS.LIMITE_CHESP(D.CONTAC, '24/08/2023')
                )
            ) AS SALDO_ATUAL,
            B.CONTAC,
            D.CONTA,
            ...
            C.LIMITE_CHESP,
            FACMUTUO.FACCOR_FUNCTIONS.NUMDIASAD(D.CONTAC, '24/08/2023') AS DIAS_AD,
            E.DESCRICAO AS NIVEL,
            FACMUTUO.FACCOR_FUNCTIONS.PEGASALDODIABLOQ(D.CONTAC, '24/08/2023') AS SALDO_BLOQ,
            F.SALDO
        FROM
            FACMUTUO.CC_CONTA D,
        ...
        WHERE
            A.COD_SITUACAO = 9
            -- A.DEVOLUCAO = 2
            AND A.DT_ATUALIZADO LIKE '%24/08/2023%'
...
LEFT JOIN (
    ...
    FROM
        ...
        JOIN FACMUTUO.CC_CHEQUE A ON A.DT_ATUALIZADO LIKE '%24/08/2023%'
        JOIN FACMUTUO.CC_TALAO B ON G.CONTAC = B.CONTAC AND B.COD_TALAO = A.COD_TALAO
    WHERE
        TO_DATE('24/08/2023') - G.VENC_FIM >= 1
