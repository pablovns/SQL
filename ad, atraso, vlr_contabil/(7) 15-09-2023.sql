...
FROM
    (
        SELECT
            consultaDados.CONTA,
            consultaDados.CPF_CNPJ,
            consultaDados.ASSOCIADO,
            consultaDados.GERENTE,
            consultaProfissao.PROFISSAO,
            consultaDados.DATA_ADMISSAO,
            consultaDados.UNID
        FROM
            (
                SELECT
                    ...
                    D.ADM_COOP DATA_ADMISSAO,
                    E.EMAIL,
                    A.FISICA,
                    D.DT_SERASA,
                    CASE
                        WHEN D.EXECUCAO = 'T' THEN D.DT_EXECUCAO
                        ELSE NULL
                    END DT_EXECUCAO,
            ...
            LEFT JOIN (
                ...
                ORDER BY
                    CONTA
            ) consultaProfissao ON consultaDados.CONTA = consultaProfissao.CONTA
        GROUP BY
            consultaDados.CONTA,
            CPF_CNPJ,
            ASSOCIADO,
        ...
        FROM
                ...
                FROM
                    (
                        SELECT
                            ...
                            (
                                -1 * (
                                    FACMUTUO.FACCOR_FUNCTIONS.PEGASALDODIA(A.CONTAC, '30/07/2023') + FACMUTUO.FACCOR_FUNCTIONS.LIMITE_CHESP(A.CONTAC, '30/07/2023')
                                )
                            ) SALDO_AD,
            ...
            OUTER JOIN (
                SELECT
                    ...
                    (
                        ...
                        WHERE
                            ...
                            AND (
                                (W.SITUACAO = 'Normal')
                                OR (W.SITUACAO = 'Afastado')
                                OR (substr(W.SITUACAO, 1, 3) = 'Dem')
                                OR (W.SITUACAO = 'Inativo')
                            )
                ...
                WHERE
                    ...
                    AND (
                        (B.SITUACAO = 'Normal')
                        OR (B.SITUACAO = 'Afastado')
                        OR (substr(B.SITUACAO, 1, 3) = 'Dem')
                        OR (B.SITUACAO = 'Inativo')
                    )
                GROUP BY
                    B.CONTA,
                    B.NIVEL
            ) consultaAtraso ON consultaAd.CONTA = consultaAtraso.CONTA
    ) consultaAdAtraso ON consultaInformacoes.CONTA = consultaAdAtraso.CONTA
ORDER BY
    ASSOCIADO